...
`genWorkenvir_gh` function. It is fully populated 
...
`setwd`) and the `systemPipeRNAseq.Rmd` source file opened that provides the code 
of this workflow. This file is located within the `rnaseq` directory.
...
# sal <- SPRproject(overwrite=TRUE) # Resets workflow, meaning previous work will be erased
...
```{r seefastqplot, eval=TRUE, warning= FALSE, echo=FALSE, out.height="150px", fig.align = "center", fig.cap= "FASTQ quality report for 18 samples.", warning=FALSE}
knitr::include_graphics("results/fastqReport.png")
```
...
30  --max-intronlen 3000  -1 ./results/preprocessing/M1A_1.fastq_trim.gz -2 ./results/preprocessing/M1A_2.fastq_trim.gz --threads 4"
...
each sample using the `featureCounts` function. 
